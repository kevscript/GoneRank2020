  ...
  Query: {
    ...
    match: async (_, { id }, req) => {
      ...
      try {
        ...
        if (!match) {
          throw new ApolloError(`Match with id ${id} doesn't exist in the DB.`)
        }
        return match
      } catch (err) {
        throw new ApolloError(err)
  ...
  Mutation: {
    ...
    removeMatch: async (_, { id }, req) => {
      ...
      try {
        ...
        if (match.lineup.length > 0) {
          const playersPromises = match.lineup.map((player) =>
            Player.findOne({ _id: player.playerId })
          )
          const players = await Promise.all(playersPromises)
          players
            .filter((p) => p !== null)
            .map((player) => {
              player.matchesPlayed = player.matchesPlayed.filter(
                (m) => m.matchId !== id
              )
              player.save()
            })
    ...
    removePlayerFromMatch: async (_, { matchId, playerId }, req) => {
      ...
      try {
        ...
        if (!match) {
          throw new ApolloError(`Match with id ${matchId} not found in DB.`)
        }
        const player = await Player.findOne({ _id: playerId })
        if (!player) {
          throw new ApolloError(`Player with id ${playerId} not found in DB.`)
        }
  ...
  Match: {
    average: (match) => {
      if (match.lineup.length > 0) {
        ...
        return parseFloat((sumOfAvgs / playersAvgs.length).toFixed(2))
      } else {
        return NaN
      }
    },
  ...
  MatchLineupPlayer: {
    average: (lineupPlayer) => {
      if (lineupPlayer.ratings.length > 0) {
        const sum = lineupPlayer.ratings.reduce(
      ...
      try {
        ...
        if (!player) {
          throw new ApolloError(
            `Couldn't find a player with id ${lineupPlayer.playerId} in the DB.`
          )
        }
        return player
      } catch (err) {
